...
const CardWrapper = ({ href, children, className }: { href?: string; children: React.ReactNode; className?: string }) =>
    href && href !== "#" ? (
...
const WideCard = ({ p, idx }: { p: Project; idx: number }) => (
    <CardWrapper href={p.link} className="md:col-span-5">
        <GlassPanel opaque className="group relative overflow-hidden flex flex-col border border-border hover:border-primary/50 transition-all duration-500 h-full cursor-pointer">
            <div className="relative overflow-hidden h-48 shrink-0">
                <ProjectImage src={p.image} alt={p.title} className="h-full w-full object-cover object-top group-hover:scale-105 transition-transform duration-700" />
            ...
            <div className="p-5 flex flex-col gap-3 flex-1">
                ...
                {p.link && p.link !== "#" && (
                    <div className="flex gap-4 pt-3 border-t border-border">
                        <span className="font-mono text-[9px] flex items-center gap-1 text-primary">DEMO <ExternalLink className="size-2.5" /></span>
                    </div>
                )}
            </div>
        </GlassPanel>
...
const MidCard = ({ p, idx }: { p: Project; idx: number }) => (
    <CardWrapper href={p.link} className="h-full">
        <GlassPanel opaque className="group relative overflow-hidden flex flex-col border border-border hover:border-primary/50 transition-all duration-500 h-full cursor-pointer">
...
const DarkCard = ({ p, idx }: { p: Project; idx: number }) => (
    <CardWrapper href={p.link} className="md:col-span-3 h-full">
        <GlassPanel background="bg-foreground" inverted className="group relative overflow-hidden bg-foreground dark:bg-card flex flex-col h-full cursor-pointer">
            <div className="absolute inset-0">
                <ProjectImage src={p.image} alt={p.title} className="h-full w-full object-cover object-top opacity-30 dark:opacity-20 group-hover:opacity-50 transition-opacity duration-500" />
            </div>
                ...
                <div className="flex-1 flex flex-col justify-end">
                    ...
                    {p.link && p.link !== "#" && (
                        <div className="flex gap-4 pt-2 border-t border-white/20">
                            <span className="font-mono text-[9px] flex items-center gap-1 text-white/60 group-hover:text-white transition-colors">DEMO <ExternalLink className="size-2.5" /></span>
                        </div>
                    )}
                </div>
            </div>
        ...
        >
            ...
            <div className="flex justify-between items-end mb-16 gap-4">
                ...
                <p className="text-muted-foreground max-w-sm text-left hidden md:block">
                    A collection of experiments, production apps, and other
                    contributions.
                </p>
            </div>

            {/* Featured Project (first one — big) */}
            <CardWrapper href={projects[0].link} className="mb-6 block">
                <GlassPanel opaque className="group relative border border-border hover:border-primary/50 transition-all duration-500 cursor-pointer">
                    <div className="grid md:grid-cols-[1fr_1fr] min-h-[360px]">
                        ...
                        >
                            <div className="relative overflow-hidden w-full h-full">
                                <ProjectImage src={projects[0].image} alt={projects[0].title} className="h-full w-full min-h-[240px] object-cover object-top" />
                        ...
                        <div className="p-8 flex flex-col justify-between relative">
                            ...
                            {projects[0].link && projects[0].link !== "#" && (
                                <div className="flex gap-6 mt-8">
                                    <span
                                        className="font-mono text-xs flex items-center gap-2 text-foreground group-hover:text-primary transition-colors group/link"
                                    >
